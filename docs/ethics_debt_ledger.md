...
| 2025-11-17 | All merchants                   | Premium-specific ML tuning could introduce fairness drift if models diverge. All merchants must share a single fraud model.                                                                         | ML Lead            | Maintain a single shared model; prohibit tier-specific features; enforce parity through model-hash comparison during CI/CD.                                                | `tests/redbar/test_model_parity.py::test_premium_uses_same_fraud_model` |
